...
public class BonangScript : MonoBehaviour {
	...
	//bool tourScript.p2SAttack1 = false;
	//bool tourScript.p2SAttack2 = false;
	//bool tourScript.p2SAttack3 = false;
	...
	void Update () {
		...
		if (Time.timeScale != 0) {
			...
			if (sAttackTimer <= 5 && sAttackTimer > 4f) {
				tourScript.p1Anim.SetInteger ("FightMove", 2);
			...
			if (tourScript.p2SAttack3Start) {
                ...
                transform.position = new Vector3 (transform.position.x + ((-6f - transform.position.x) * 0.4f), transform.position.y, transform.position.z);

				if (tourScript.swooshSound.isPlaying == false) {
					tourScript.swooshSound.Play ();
				}

				Instantiate (Resources.Load ("Prefabs/BonangShadow"), new Vector3 (transform.position.x, transform.position.y, transform.position.z), Quaternion.identity);
			}
		
			/*if (tourScript.p2SAttack3Timer == 5) {
			...
			if (p1SAttackTimer < p1SAttackTimerMax * 0.5f && p1SAttackTimer > ((p1SAttackTimerMax * 0.5f)-1f) ) { 
				...
				//if (tourScript.p2Def1Active.GetComponentInChildren<RectTransform>().anchoredPosition.y == -72f && tourScript.p2Def2Active.GetComponentInChildren<RectTransform>().anchoredPosition.y == -72f){
...
//				Instantiate (Resources.Load ("Prefabs/PunchClash3"), new Vector3 (transform.position.x, transform.position.y, transform.position.z), Quaternion.identity);
			...
			if (standUpStart) {
                standUpTimer -= Time.deltaTime * 50f;
			}
			...
			if (ending) {
                endingTimer -= Time.deltaTime * 50f; ;
			}
