...
public class NKScript : MonoBehaviour {
	...
	void Update () {
		...
		if (Time.timeScale != 0) {
			...
			if (flyOutTimer <= 0) {
				...
				flyOutTimer = flyOutTimerMax;
			}

			if (flyOut) {
				//tourScript.p2Anim.SetInteger ("FightMove", 19);
				transform.position = new Vector3 (4, transform.position.y + (10 - transform.position.y) * 0.03f, 0);
			}

			// --- P2 Loses ---
			if (tourScript.p2HBar.GetComponent<RectTransform> ().rect.width <= 0) {
				if (tourScript.tournamentNumb == 1) {
					tourScript.matchNumb = 0;
					PlayerPrefs.SetInt ("registered", 0);
				} else if (tourScript.tournamentNumb == 2 || tourScript.tournamentNumb == 3 || tourScript.tournamentNumb == 4) {
					if (tourScript.matchNumb != 0) {
						tourScript.matchNumb = 3;
					}
				}
			}
