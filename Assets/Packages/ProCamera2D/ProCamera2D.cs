...
namespace Com.LuisPedroFonseca.ProCamera2D
{
    ...
    public class ProCamera2D : MonoBehaviour
    {
        ...
        public void AddCameraTargets(IList<Transform> targetsTransforms, float targetsInfluenceH = 1f, float targetsInfluenceV = 1f, float duration = 0f)
        {
            for (int i = 0; i < targetsTransforms.Count; i++)
            {
                AddCameraTarget(targetsTransforms[i], targetsInfluenceH, targetsInfluenceV, duration);
            }
        }
        ...
        void SetScreenSize(float newSize)
        {
            ...
            else
            {
                if (ZoomWithFOV)
                {
                    GameCamera.fieldOfView = 2f * Mathf.Atan(newSize / Mathf.Abs(Vector3D(_transform.localPosition))) * Mathf.Rad2Deg;
                }
                else
                {
